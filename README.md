...
<body>
    <div class="container">
        ...
        <div class="model">
            <h2 class="model-title">模型二：不同磁性磁铁</h2>
        ...
        <div class="explanation">
            ...
            <ul>
                ...
                <li><strong>弹簧偏移</strong> 在释放后弹簧会偏向强磁性磁铁</li>
            </ul>
        </div>
    </div>
    ...
    <script>
        ...
        // 模型参数：不同磁性磁铁
        let modelState = {
            magnetA: { x: 150, y: 110, radius: 38, color: '#e74c3c', fixed: true, strength: 2.5 },
            magnetB: { x: 450, y: 110, radius: 22, color: '#3498db', fixed: true, strength: 0.5 },
            spring: { 
                length: 140, 
                k: 0.08, 
                centerOffset: 0,
        ...
        function calculateForces() {
            ...
            if (modelState.isCompressing) {
                ...
                if (modelState.released) {
                    const compressionRatio = 1 - (modelState.magnetB.x - modelState.magnetA.x - modelState.magnetA.radius - modelState.magnetB.radius) / modelState.spring.naturalLength;
                    const strengthRatio = modelState.magnetA.strength / (modelState.magnetA.strength + modelState.magnetB.strength);
                    modelState.spring.centerOffset = -0.3 * strengthRatio * (1 - compressionRatio * 0.5);
                }
            }
            
            // 释放后弹簧中心偏移
            if (!modelState.magnetB.fixed && modelState.released && !modelState.isCompressing) {
                const strengthRatio = modelState.magnetA.strength / (modelState.magnetA.strength + modelState.magnetB.strength);
                modelState.spring.centerOffset = -0.3 * strengthRatio;
            }
        ...
        function resetModel() {
            ...
            modelState.released = false;
        }
        
        // 释放磁铁B
        function releaseModel() {
            modelState.magnetB.fixed = false;
            modelState.released = true;
        }
